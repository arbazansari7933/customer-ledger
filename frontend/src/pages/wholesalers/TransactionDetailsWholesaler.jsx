...
const TransactionDetailsWholesaler = () => {
  const { transactionId } = useParams();
  const location = useLocation();
  const wholesalerId = location.state?.wholesalerId;
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const res = await api.get(`/wholesalers/${wholesalerId}/transactions/${transactionId}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await api.delete(`/wholesalers/${wholesalerId}/transactions/${transactionId}`);
      navigate(`/wholesaler-details/${wholesalerId}`)
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 px-4 sm:px-6 py-6">
      ...
      <div className="mb-6 w-full max-w-xl mx-auto bg-white shadow-sm rounded-xl p-5 sm:p-6">
        ...
        <div className="flex justify-between items-start mb-3">
          ...
          {/* Back Button */}
          <button
            onClick={() => navigate(`/wholesaler-details/${wholesalerId}`)}
          ...
          <div className="flex gap-2">
            <Link
              to={`/wholesaler/transaction-detail/edit/${transactionId}`}
              state={{ type: transactionType, wholesalerId }}
        ...
        <p className="text-gray-600 text-base">
          <span className="font-semibold">Type:</span> {(data.type=="give" ? "Payment Diya" : "Stock Ayaa")}
        </p>
...
export default TransactionDetailsWholesaler
